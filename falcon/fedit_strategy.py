"""Flower strategy for the FedIT / FedAvg-LoRA baseline.

FedIT uses a fixed LoRA rank on every client, asks all clients to upload the
full adapter (A and B), and applies sample-weighted FedAvg directly to both
factors.
"""
...
class FedITStrategy(fl.server.strategy.Strategy):
    def __init__(self, config: Config, client_ranks: Dict[int, int]):
        self.config = config
        self.client_ranks = client_ranks
        self._validate_fixed_rank()
        self.global_blob = None
        self.per_client_eval: Dict[int, Dict[int, float]] = {}

    def _validate_fixed_rank(self) -> None:
        ranks = {int(rank) for rank in self.client_ranks.values()}
        if len(ranks) != 1:
            raise ValueError(
                "FedIT fixed baseline requires one shared rank for all clients. "
                "Set rank_pool to a single value, for example [8]."
            )
    ...
    def configure_fit(self, server_round: int, parameters: Parameters,
        ...
        print(f"[FedIT] round {server_round}: requesting A+B from all clients")
        return instructions
    ...
    def aggregate_fit(self, server_round: int,
        ...
        self.global_blob = self._average_full_adapter_blob(payloads)

        comm_cost = self._communication_cost(payloads)
        print(f"[FedIT] round {server_round}: averaged full adapters, "
    ...
    def aggregate_evaluate(self, server_round: int,
        ...
        print(f"[FedIT] round {server_round}: mean eval loss = {mean_loss:.4f}")
    ...
    def _validate_full_upload(self, payloads, server_round: int) -> None:
        ...
        if missing:
            raise RuntimeError(
                f"round {server_round}: FedIT requires A+B from every client, "
    ...
    def _average_full_adapter_blob(self, payloads):
        ...
        for key in self._layer_keys(payloads):
            first_a = payloads[0]["layers"][key]["A"]
            first_b = payloads[0]["layers"][key]["B"]
            a_global = np.zeros_like(first_a, dtype=np.float64)
            b_global = np.zeros_like(first_b, dtype=np.float64)
            for payload, weight in zip(payloads, weights):
                a_mat = payload["layers"][key]["A"]
                b_mat = payload["layers"][key]["B"]
                if a_mat.shape != first_a.shape:
                    raise ValueError(
                        "FedIT fixed-rank aggregation requires matching A shapes "
                        f"for layer {key}: expected {first_a.shape}, got {a_mat.shape}"
                    )
                if b_mat.shape != first_b.shape:
                    raise ValueError(
                        "FedIT fixed-rank aggregation requires matching B shapes "
                        f"for layer {key}: expected {first_b.shape}, got {b_mat.shape}"
                    )
                a_global += weight * a_mat
                b_global += weight * b_mat
            blob[key] = (
                a_global.astype(first_a.dtype, copy=False),
                b_global.astype(first_b.dtype, copy=False),
            )
        return blob
