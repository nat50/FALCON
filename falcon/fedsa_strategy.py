"""Flower strategy for the FedSA baseline.

FedSA keeps LoRA B local on every client and aggregates only the shared A
factor. This implementation intentionally avoids FALCON's agent, B-selection,
and dynamic-rank allocation so the baseline remains fixed-rank and A-only.
"""
...
class FedSAStrategy(fl.server.strategy.Strategy):
    ...
    def _validate_fixed_rank(self) -> None:
        ...
        if len(ranks) != 1:
            raise ValueError(
                "FedSA requires a fixed rank for all clients. "
    ...
    def configure_fit(self, server_round: int, parameters: Parameters,
        ...
        print(f"[FedSA] round {server_round}: requesting A only")
        return instructions
    ...
    def aggregate_fit(self, server_round: int,
        ...
        self._validate_a_only(payloads, server_round)
        self.global_blob = self._average_a_only_blob(payloads)

        comm_cost = self._communication_cost(payloads)
        print(f"[FedSA] round {server_round}: merged A only, "
    ...
    def aggregate_evaluate(self, server_round: int,
        ...
        print(f"[FedSA] round {server_round}: mean eval loss = {mean_loss:.4f}")
    ...
    def _validate_a_only(self, payloads, server_round: int) -> None:
        unexpected = sorted(
            p["client_id"] for p in payloads
            if p["request_b"] or self._has_B(p)
        )
        if unexpected:
            raise RuntimeError(
                f"round {server_round}: FedSA received B from clients "
                f"{unexpected}, but FedSA must upload A only"
            )

    def _layer_keys(self, payloads) -> List[str]:
        return list(payloads[0]["layers"].keys())

    def _average_a_only_blob(self, payloads):
        ...
        for key in self._layer_keys(payloads):
            ...
            for payload, weight in zip(payloads, weights):
                ...
                if a_mat.shape != first_a.shape:
                    raise ValueError(
                        "FedSA fixed-rank aggregation requires matching A shapes "
                        f"for layer {key}: expected {first_a.shape}, got {a_mat.shape}"
                    )
                a_global += weight * a_mat
            blob[key] = (a_global.astype(first_a.dtype, copy=False), None)
        return blob

    def _communication_cost(self, payloads) -> float:
        return float(sum(p["rank"] for p in payloads))
